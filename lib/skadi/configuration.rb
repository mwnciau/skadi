module Skadi
  class Configuration
    ...
    validates(:use_anonymisation_sets, "boolean") { |it| it == true || it == false }

    # How long an anonymisation set should last before expiring. Defaults to 1 day.
    # @return [ActiveSupport::Duration]
    attr_accessor :anonymisation_set_duration
    validates(:anonymisation_set_duration, "ActiveSupport::Duration") { |it| it.is_a?(ActiveSupport::Duration) }
    ...
    validates(:anonymisation_set_reset_hour, "Integer or false") do |it|
    ...
    validates(:visit_duration, "ActiveSupport::Duration") { |it| it.is_a?(ActiveSupport::Duration) }

    # The parent app's User class, used to link visits to users
    # @return [Class, nil]
    attr_accessor :user_model
    validates(:user_model, "Class or nil") { |it| it.nil? || (it.is_a?(Class) && it < ActiveRecord::Base) }

    # Method to call within controllers to get the current user.
    # @return [Symbol, nil]
    attr_accessor :user_method
    validates(:user_method, "Symbol or nil") { |it| it.nil? || it.is_a?(Symbol) }
    ...
    validates(:use_query_param_whitelist, "boolean") { |it| it == true || it == false }

    # An array of query parameter keys to whitelist for storage in URLs.
    # @return [Array<Symbol>]
    attr_accessor :query_param_whitelist
    validates(:query_param_whitelist, "Array<Symbol>") { |it| it.is_a?(Array) && it.all? { |key| key.is_a?(Symbol) } }
    ...
    validates(:db_connects_to, "Hash compatible with ActiveRecord::ConnectionHandling#connects_to") do |it|
      next true if it.nil?
    ...
    validates(:store_domain_in_views, "boolean") { |it| it == true || it == false }

    # Sets a limit on the size of the tracking beacon. Defaults to 5KB.
    # @return [Integer]
    attr_accessor :max_tracking_payload_size
    validates(:max_tracking_payload_size, "integer") { |it| it.is_a?(Integer) && it > 0 }
    ...
    validates(:cookie_domain, "string or nil") { |it| it.nil? || (it.present? && it.is_a?(String)) }
